...
func getExternalProduct(id string) (bool, error) {
	...
	enpoint := fmt.Sprintf("http://%s:%s/api/v1/product/%s", publicIP, "3000", realId)
	fmt.Println(enpoint)
...
func postExternalOrder(order pb.OrderResult) {
	...
	publicIP := os.Getenv("PUBLIC_IP")
	fmt.Println(publicIP)
	request, err := http.NewRequest("POST", fmt.Sprintf("http://%s:%s/api/v1/order", publicIP, "3000"), bytes.NewBuffer(jsonData))
	if err != nil {
